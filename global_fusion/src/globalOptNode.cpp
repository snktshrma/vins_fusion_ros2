...
void publish_car_model(double t, Eigen::Vector3d t_w_car, Eigen::Quaterniond q_w_car)
{
    ...
    car_mesh.mesh_resource = car_mesh_resource;
...
void vio_callback(const nav_msgs::msg::Odometry::SharedPtr pose_msg)
{
    ...
    if (!output_csv_path.empty()) {
      std::ofstream foutC(output_csv_path, std::ios::app);
      const int64_t t_ns = static_cast<int64_t>(pose_msg->header.stamp.sec) * 1000000000LL +
                           static_cast<int64_t>(pose_msg->header.stamp.nanosec);
      foutC.setf(std::ios::fixed, std::ios::floatfield);
      foutC.precision(0);
      foutC << t_ns << ",";
      foutC.precision(5);
      foutC << global_t.x() << "," << global_t.y() << "," << global_t.z() << ","
            << global_q.w() << "," << global_q.x() << "," << global_q.y() << ","
            << global_q.z() << std::endl;
    }
}
...
int main(int argc, char **argv)
{
    ...
    n->declare_parameter<std::string>("output_csv", "");
    world_frame_id = n->get_parameter("world_frame_id").as_string();
    body_frame_id = n->get_parameter("body_frame_id").as_string();
    output_csv_path = n->get_parameter("output_csv").as_string();
    globalEstimator.world_frame_id = world_frame_id;

    const std::string pkg_share =
        ament_index_cpp::get_package_share_directory("vins_fusion_ros2");
    car_mesh_resource = "file://" + pkg_share + "/models/car.dae";
